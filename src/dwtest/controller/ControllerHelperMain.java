...
public class ControllerHelperMain extends HelperBase {
	...
	public String btnConfirm() {
		...
		EstadoCivil ecivil = (EstadoCivil)HibernateHelper.getKeyData(EstadoCivil.class, edo.getCve_estado_civil());
		persona.setCve_estado_civil(ecivil);
		
		System.out.println(ecivil);
		
		Municipio muni = (Municipio)HibernateHelper.getKeyData(Municipio.class, municipio.getCve_municipio());
		dom.setCve_municipio(muni);
		...
		if (isValid(personaDatos)) {
			...
			//HibernateHelper.saveDB(ecivil);
			//HibernateHelper.saveDB(muni);
